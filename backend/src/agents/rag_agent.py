...
def _clean_repetitive_text(text: str) -> str:
    """Removes runaway repetitive character/word loops generated by LLM tokenizers."""
    if not text:
        return ""
    # Remove loops of repeating phrases or characters (3 or more consecutive identical chunks)
    cleaned = re.sub(r'(\b[\S\s]{1,20}?\b)(?:\s*\1){3,}', r'\1', text)
    # Remove repetitive single token spam at the end
    cleaned = re.sub(r'(?:[\u0D80-\u0DFF\w]{1,8}\s*){10,}$', '', cleaned).strip()
    return cleaned
...
"""
        ...
        answer = await asyncio.to_thread(self._invoke_llm, system_prompt, user_query)
